...
#Fig. 3, S8 
#pseudobulk_exp2_blood

exp2_blood  <- readRDS("./exp2_blood_combined_integrated_annotated.rds")


expr <- list()
for (cluster in unique(Idents(exp2_blood))) {
  for (sample in unique(exp2_blood@meta.data$orig.ident)) {
    cells <- Cells(exp2_blood)[(exp2_blood@meta.data$orig.ident==sample) & (Idents(exp2_blood)==cluster)]
    if (length(cells) > 5) { 
      expr[[paste0(cluster,'_',sample)]] <- rowSums(exp2_blood@assays$RNA@counts[,cells])
    }
  }
}
for (sample in unique(exp2_blood@meta.data$orig.ident)) {
  cells <- Cells(exp2_blood)[(exp2_blood@meta.data$orig.ident==sample)]
  expr[[paste0('all_',sample)]] <- rowSums(exp2_blood@assays$RNA@counts[,cells])
}
...
clusters_to_check=c('Classical monocyte', 'Non-classical monocyte', 'Neutrophil', 'Immature neutrophil', 'mDC', 'NK', 'T', 'B', 'Platelet')
...
pseudobulk_exp2_blood <- do.call(rbind,res)

write.table(pseudobulk_exp2_blood, "./pseudobulk_exp2_blood.txt", row.names = TRUE, sep="\t", quote=FALSE)


clusters=c('Classical monocyte', 'Neutrophil', 'Immature neutrophil', 'T', 'B')
contrasts_to_use <- c("adeno2xvsuntr", "att2xvsuntr", "mRNA2xvsuntr", "mRNAattvsuntr")

#Option 1: select specfici genes
genes <- c("Irf7", "Mx2", "Isg15", "Ifit3", "Lgals3bp", "Cxcl10", "Ccl5", "Tnfsf10")
the_filename="pseudobulk_exp2_blood_goi_almost_all_facet"


#Option 2: To select top x genes per contrast, do:
genes <- pseudobulk_exp2_blood %>%
...
the_filename="pseudobulk_exp2_blood_top10_almost_all_facet"


#Make plot
df <- pseudobulk_exp2_blood %>%
  dplyr::filter(cluster %in% clusters) %>%
  dplyr::filter(contrast %in% contrasts_to_use) %>%
...
#save 8x12 inches
ggsave(paste("exp2_blood", the_filename, "pdf", sep="."), width=7, height = length(genes)/6+2, units="in", useDingbats=FALSE)
